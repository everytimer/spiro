...
float cell::envelope::process()
{
        return fEnv[state](this);
}

void cell::envelope::start()
{
    if(curve==CUB)
    {
        form[0] = 1;
...
float cell::envelope::coefficient(float start_value, float end_value, uint length) 
{
