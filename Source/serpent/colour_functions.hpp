...
inline uint SET_ALPHA(uint colour, uint alpha)
{
...
inline uint SET_RGBA(uint r= 0, uint g= 0, uint b= 0, uint a= 0)
{
    uint res= 0;
    res += r; res <<= 8;
    res += g; res <<= 8;
...
inline uint SET_ARGB(uint a= 0, uint r= 0, uint g= 0, uint b= 0)
{
    uint res= 0;
    res += a; res <<= 8;
    res += r; res <<= 8;
...
inline uint SET_ABGR(uint a= 0, uint b= 0, uint g= 0, uint r= 0)
{
    uint res= 0;
    res += a; res <<= 8;
    res += b; res <<= 8;
...
inline RGBA GET_RGBA(uint colour)
{
    RGBA res;
...
inline uint DIM_COLOUR(uint colour, float factor)
{
...
inline uint DIM_ALPHA(uint colour, float factor)
{
    if(factor > 1.0f) return colour;
    if(factor < 0.0f) return 0;
...
inline uint ADD(uint colour_a, uint colour_b)
{
